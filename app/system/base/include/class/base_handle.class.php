...
class base_handle extends handle
{
    ...
    {
        if (!isset($admin_id)) return '';

        if (isset($this->publish_list[$admin_id])) {
            return $this->publish_list[$admin_id];
        }

        $admin_member = load::mod_class("admin/admin_database", 'new')->getOneByName($admin_id);
        if ($admin_member) {
            $admin_name = $admin_member['admin_name'];
            $this->publish_list[$admin_id] = $admin_name;
            return $admin_name;
        } else {
            $this->publish_list[$admin_id] = '';
            return '';
        }
    }
