...
class html extends admin
{
     ...
     * 根据静态页生成记录删除静态文件
     */
    protected function DelHtmlBylogs()
    {
        global $_M;
        $logs = $this->html_handle->getLogs($_M['lang']);
        ...
        $this->html_handle->delLogs($_M['lang']);
        return;
    }
    ...
    public function doCreatePage()
    {
        ...
        //静态页列表写入缓存
        $this->html_handle->htmlLogs($pages);
        ...
        $redata = array();
        $redata['total'] = $total;
        $redata['callback_url'] = $_M['url']['web_site'] . "app/system/entrance.php?n=html&c=html&a=doLoop&lang={$_M['lang']}&metinfonow={$_M['config']['met_member_force']}";
    ...
    {
        global $_M;
        $pages = $this->html_handle->htmlCache('get',"static_list_err_" . $_M['lang']);
        sleep(1);
        if (!$pages) {
    ...
    public function doCheckPage()
    {
        ...
        $suc = $this->html_handle->htmlCache('get',"static_list_suc_" . $_M['lang']);
        if (!$suc) {
            $suc = array();
        }

        $err = $this->html_handle->htmlCache('get',"static_list_err_" . $_M['lang']);
        if (!$err) {
            $err = array();
        }
